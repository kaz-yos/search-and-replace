...
=counsel-git-grep= is very similar except that the back end is =git=, and it is project-folder-aware.

** Example Workflow

- =Meta-x counsel-ag= to call with an empty string for the current directory. The universal argument (=Ctrl-u= before invocation) make it ask for the directory.
